module MediaHelper
  ...
  def media_create_directory(dir, url = '')
    begin
      AWS::S3::S3Object.store(url + "#{dir}/", url + "#{dir}/", BUCKET, :access => :public_read, :content_type => 'binary/octet-stream')
  ...
  def upload_images(p, type, initial_folder = 'users')

    where = "#{BUCKET}/" + initial_folder.to_s + "/" + type.to_s + "/"

    begin

      @file = AWS::S3::S3Object.store(sanitize_filename(p.original_filename), p.read, where, :access => :public_read)
      S3Object.url_for("/" + initial_folder.to_s + "/" + type.to_s + "/" + p.original_filename, BUCKET, :authenticated => false)
